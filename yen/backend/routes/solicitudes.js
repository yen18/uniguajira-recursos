...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        if (!id_usuario || !fecha || !hora_inicio || !hora_fin || !servicio || !salon || !asignatura || !docente || !celular) {
            return res.status(400).json({
                success: false,
                message: 'Campos requeridos: id_usuario, fecha, hora_inicio, hora_fin, servicio, salon, asignatura, docente, celular'
            });
        }
        ...
        if (!['videoproyector', 'sala'].includes(servicio)) {
            return res.status(400).json({
                success: false,
                message: 'El servicio debe ser "videoproyector" o "sala"'
            });
        }
        ...
        try {
            ...
            const [result] = await connection.execute(
                ...
                (id_usuario, id_sala, id_videoproyector, fecha, hora_inicio, hora_fin, 
                 ...
                 estado_reserva, salon) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'aprobado', ?)`,
                [id_usuario, recursoDisponible.id_sala, recursoDisponible.id_videoproyector, 
                 fecha, hora_inicio, hora_fin, estudiante, programa, tipo_actividad || null, numero_asistentes || null, asignatura, docente, 
                 semestre, celular, recursoDisponible.tipo, salon]
            );
            ...
            const response = {
                ...
                data: {
                    ...
                    recurso_asignado: {
                        ...
                        ubicacion: recursoDisponible.recurso.ubicacion,
                        id: recursoDisponible.tipo === 'sala' ? recursoDisponible.id_sala : recursoDisponible.id_videoproyector
                    }
                }
            };
    ...
    try {
        ...
        const [result] = await pool.execute(
            ...
            [fecha, hora_inicio, hora_fin, estudiante, programa, tipo_actividad || null, numero_asistentes || null, asignatura, 
             docente, semestre, celular, salon, id]
        );
...
router.patch('/:id/estado', async (req, res) => {
    try {
        ...
        if (!['aprobado', 'rechazado', 'pendiente'].includes(finalEstado)) {
            return res.status(400).json({
                success: false,
        ...
        if (process.env.DATE_STRICT === '1' && /^\d{4}-\d{2}-\d{2}$/.test(fecha)) {
            try {
                ...
                if (diffDays === -1 && now.getHours() >= 6) {
                    return res.status(400).json({ success: false, error: 'fecha_desfasada', message: 'La fecha enviada corresponde al día anterior. Selecciona la fecha actual antes de enviar.' });
                }
