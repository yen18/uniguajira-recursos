...
const Usuarios = ({ user }) => {
  ...
  const { data: usuariosData, isLoading, isError, error: qError, refetch } = useQuery({
    queryKey: ['usuarios'],
    queryFn: async () => {
  ...
  React.useEffect(() => {
    setUsuarios(asArray(usuariosData));
    setLoading(isLoading);
    if (isError) setError(handleApiError(qError).message);
  }, [usuariosData, isLoading, isError, qError]);
        ...
        <CardContent>
          <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
            <Typography variant="h6">
              Lista de Usuarios ({Array.isArray(usuarios) ? usuarios.length : 0})
            </Typography>
